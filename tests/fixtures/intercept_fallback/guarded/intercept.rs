...
// diverting anything containing "blocked" *and* the bare boundary path itself
// (which the nest strips to "/" or ""), letting the rest reach the fallback.
...
pub async fn intercept(req: Request) -> ControlFlow<Response, Request> {
    ...
    if path.contains("blocked") || path == "/" || path.is_empty() {
